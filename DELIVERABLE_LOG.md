...
A running log of **reusable IDO workflows** agents have built for end users in this repo. Each entry records:

- **Request** — what the user needed
- **Discovery path** — how the agent found the solution
- **Deliverable** — the final API call(s)

Append a new entry at the top after completing any non-trivial IDO task. Scope is limited to user-facing API work — don't log repo maintenance, refactors, doc edits, tool installation, or validation runs.
